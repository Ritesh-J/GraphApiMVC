...
namespace GraphApiMVC
{
    public class OneDriveService
    {
        ...
        {
            Console.WriteLine("inside service before try");
            try
            {
                Console.WriteLine("inside service after try");
                var graphClient = new GraphServiceClient(new DelegateAuthenticationProvider((requestMessage) =>
                {
                    requestMessage.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", accessToken);
                    return Task.CompletedTask;
                }));
                Console.WriteLine("graphClient Client passed");
                // Read the file content
                byte[] fileContent = await System.IO.File.ReadAllBytesAsync(filePath);
                Console.WriteLine("read the file contet");
